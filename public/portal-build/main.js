...
function notificationCreditId(item) {
...
function mergeNotificationItems(items) {
    ...
    for (const item of notifItems)
        merged.set(notificationKey(item), item);
...
function isActionableCredit(item) {
    if (!item.data?.creditId || item.data.consumedAt || item.data.activatable !== true)
        return false;
    if (!item.data.expiresAt)
        return true;
    return new Date(item.data.expiresAt).getTime() > Date.now();
}
function creditExpiryLabel(item) {
    if (!item.data?.expiresAt)
        return "Activate when ready";
...
function renderNotifications() {
    ...
    notifList.innerHTML = notifItems.length
        ? notifItems
            .map((item) => `<div class="notif-item ${item.readAt ? "read" : ""}">
...
async function loadRedeemableCredits(username = notifUsername) {
    ...
    if (state.hotspot?.mac)
        params.set("deviceMac", state.hotspot.mac);
    if (!params.toString())
        return;
    try {
...
async function activateCredit(creditId) {
    if (!creditId || activatingCreditId)
        return;
    if (!state.hotspot?.mac) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(payload.error || "Unable to activate this credit.");
...
function handleCreditActivationClick(event) {
    const target = event.target;
    if (!(target instanceof HTMLElement))
        return;
...
document.addEventListener("click", (event) => {
    ...
    if (!(target instanceof HTMLElement))
        return;
    if (target.closest(".notif-control"))
        return;
...
function promotedBadge(plan) {
    ...
    if (speedTierForPlan(plan)?.key === "gulfstream")
        return "Top speed";
    return "";
}
...
function speedTierPitch(plan) {
    ...
    if (tierKey === "gulfstream")
        return "Top-speed access for heavy downloads, uploads, and urgent work.";
    if (tierKey === "fast")
            ...
            if (!response.ok) {
                ...
                if (response.status === 400 && payload?.entitlement) {
                    ...
                    if (stillActive) {
                        ...
                        showPaymentModal("welcome-used", "Nah bro \u{1F62D}", "Someone already used your welcome access on this device. It's a one-time thing — grab one of the packages below instead.");
                    }
                    return;
                }
